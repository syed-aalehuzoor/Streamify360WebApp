<tr class="border-b hover:bg-gray-50 h-16" @if ($video->status !== 'live') wire:poll.2s="updatevideo" @endif>
    ...
    <td class="p-3">
        <div class="flex justify-between group">
            <a @if ($video->status === 'live') href="{{ route('video.player', $video->id) }}" @else title="Video is Being Processed. Check back in a Minute!" @endif target="_blank" class="text-blue-500 text-sm hover:underline cursor-pointer">
                {{ $video->name }}
            </a>                
